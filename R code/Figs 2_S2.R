...
##########################################################
## BC transform PO4, NO3, K; works for PO4 and K, but not for NO3
library(MASS)
bcTransPO4 <- boxcox(lm(dataForStats$PO4.dep ~ 1), lambda = seq(-2, 2, 1/20), plotit = F)
bcTransNO3 <- boxcox(lm(dataForStats$NO3.dep ~ 1), lambda = seq(-2, 2, 1/20), plotit = F)
bcTransK <- boxcox(lm(dataForStats$K.dep ~ 1), lambda = seq(-2, 2, 1/20), plotit = F)
detach(package:MASS)
lambdaPO4 <- bcTransPO4$x[which.max(bcTransPO4$y)]
lambdaNO3 <- bcTransNO3$x[which.max(bcTransNO3$y)]
lambdaK <- bcTransK$x[which.max(bcTransK$y)]
tempPO4 <- dataForStats %>% mutate(bcPO4 = (PO4.dep^lambdaPO4-1)/lambdaPO4)
tempNO3 <- dataForStats %>% mutate(bcNO3 = (NO3.dep^lambdaNO3-1)/lambdaNO3)
tempK <- dataForStats %>% mutate(bcK = (K.dep^lambdaK-1)/lambdaK)
bc.lme.PO4 <- lmer(data = tempPO4, bcPO4 ~ LAI.4.5 + (N*P):LAI.4.5 + (N*K):LAI.4.5 + (P*K):LAI.4.5 + (1|Replicate/Block/Plot) + (1|Collection))
bc.lme.NO3 <- lmer(data = tempNO3, bcNO3 ~ LAI.4.5 + (N*P):LAI.4.5 + (N*K):LAI.4.5 + (P*K):LAI.4.5 + (1|Replicate/Block/Plot) + (1|Collection))
bc.lme.K <- lmer(data = tempK, bcK ~ LAI.4.5 + (N*P):LAI.4.5 + (N*K):LAI.4.5 + (P*K):LAI.4.5 + (1|Replicate/Block/Plot) + (1|Collection))

p7 <- resid_panel(bc.lme.PO4, plots = c("resid", "qq", "hist"), nrow = 1)
p8 <- resid_panel(bc.lme.NO3, plots = c("resid", "qq", "hist"), nrow = 1)
p9 <- resid_panel(bc.lme.K, plots = c("resid", "qq", "hist"), nrow = 1)
bcResid <- grid.arrange(p7, p8, p9)
#ggsave("/Users/palaniakana/Desktop/BC Dep Resid Plots.png", bcResid, width = 6, height = 5)


##########################################################
## Full dep and conc models for other nutrients
dataForStats <- dataForStats %>% mutate(cat.dep = Mg.dep + K.dep + Ca.dep + NH4.dep + Na.dep,
                                        cat.charge.dep = 2*Mg.dep + K.dep + 2*Ca.dep + NH4.dep + Na.dep,
                                        cat.conc = Mg_ppm + K_ppm + Ca_ppm + Na_ppm + NH4_ppm,
                                        cat.charge.conc = 2*Mg_ppm + K_ppm + 2*Ca_ppm + Na_ppm + NH4_ppm)
dep.lme.Na <- lmer(data = dataForStats, log(Na.dep) ~ LAI.4.5 + (N*P):LAI.4.5 + (N*K):LAI.4.5 + (P*K):LAI.4.5 + (1|Replicate/Block/Plot))
dep.lme.cats <- lmer(data = dataForStats, log(cat.dep) ~ LAI.4.5 + (N*P):LAI.4.5 + (N*K):LAI.4.5 + (P*K):LAI.4.5 + (1|Replicate/Block/Plot))
conc.lme.Na <- lmer(data = dataForStats, log(Na_ppm) ~ LAI.4.5 + log(Volume_mL) + (N*P):LAI.4.5 + (N*K):LAI.4.5 + (P*K):LAI.4.5 + (1|Replicate/Block/Plot))
conc.lme.cats <- lmer(data = dataForStats, log(cat.conc) ~ LAI.4.5 + log(Volume_mL) + (N*P):LAI.4.5 + (N*K):LAI.4.5 + (P*K):LAI.4.5 + (1|Replicate/Block/Plot))

summary(dep.lme.Na) ## N.S.
summary(dep.lme.cats) ## +LAI
summary(conc.lme.Na) ## -Volume
summary(conc.lme.cats) ## +LAI, -Volume

## Fe, Cu, Zn, Mn, Mo all close to zero (detection limit = 0.01)
mean(dataForStats$Fe_ppm, na.rm = T); mean(dataForStats$Cu_ppm, na.rm = T)
mean(dataForStats$Zn_ppm, na.rm = T); mean(dataForStats$Mn_ppm, na.rm = T)
mean(dataForStats$Mo_ppm, na.rm = T)


##########################################################
## Models without LAI (fertilization effect only)
noLAI.lme.PO4 <- lmer(data = dataForStats, log(PO4.dep) ~ N*P + P*K + N*K + (1|Replicate/Block/Plot) + (1|Collection))
noLAI.lme.NH4 <- lmer(data = dataForStats, log(NH4.dep) ~ N*P + P*K + N*K + (1|Replicate/Block/Plot) + (1|Collection))
noLAI.lme.NO3 <- lmer(data = dataForStats, log(NO3.dep) ~ N*P + P*K + N*K + (1|Replicate/Block/Plot) + (1|Collection))
noLAI.lme.N <- lmer(data = dataForStats, log(N.dep) ~ N*P + P*K + N*K + (1|Replicate/Block/Plot) + (1|Collection))
noLAI.lme.K <- lmer(data = dataForStats, log(K.dep) ~ N*P + P*K + N*K + (1|Replicate/Block/Plot) + (1|Collection))
noLAI.lme.Mg <- lmer(data = dataForStats, log(Mg.dep) ~ N*P + P*K + N*K + (1|Replicate/Block/Plot))
noLAI.lme.Ca <- lmer(data = dataForStats, log(Ca.dep) ~ N*P + P*K + N*K + (1|Replicate/Block/Plot))

## Compare R2-values of deposition with and without LAI as covariate
## Marginal R2s (fixed effects) listed below: usually slightly better with LAI, but never more than 0.1 better
library(MuMIn)
r.squaredGLMM(dep.lme.PO4); r.squaredGLMM(noLAI.lme.PO4) ## 0.42 v 0.36
r.squaredGLMM(dep.lme.NH4); r.squaredGLMM(noLAI.lme.NH4) ## 0.057 v 0.049
r.squaredGLMM(dep.lme.NO3); r.squaredGLMM(noLAI.lme.NO3) ## 0.073 v 0.033
r.squaredGLMM(dep.lme.N); r.squaredGLMM(noLAI.lme.N) ## 0.037 v 0.038
r.squaredGLMM(dep.lme.K); r.squaredGLMM(noLAI.lme.K) ## 0.13 v 0.028
r.squaredGLMM(dep.lme.Mg); r.squaredGLMM(noLAI.lme.Mg) ## 0.19 v 0.12
r.squaredGLMM(dep.lme.Ca); r.squaredGLMM(noLAI.lme.Ca) ## 0.21 v 0.16


##########################################################
## Deposition: summing over plot
dataForPlots %>% group_by(Plot, Collection) %>%
  summarize(meanPO4 = mean(PO4.dep, na.rm = T), Treatment = first(Treatment), P = first(P.fert)) %>%
  ungroup() %>% group_by(Plot) %>% arrange(Plot) %>%
  summarize(totalPO4 = sum(meanPO4, na.rm = T), Treatment = first(Treatment), P = first(P)) %>%
  ggplot(aes(x = Treatment, y = totalPO4, color = P)) +
  geom_boxplot() + 
  geom_point() + theme_bw()
